    ...
    private async selectNetBankingAndCheckout(
            ...
            for (const { frame } of checkFrames) {
                try {
                    const open = await safeEval(frame, () => {
                        const isVis = (e: any) => { if (!e.isConnected) return false; const s = window.getComputedStyle(e); return s.display !== 'none' && s.visibility !== 'hidden' && s.visibility !== 'collapse' && s.opacity !== '0' && !e.hasAttribute('hidden') && e.getAttribute('aria-hidden') !== 'true' && e.getBoundingClientRect().width > 0; };
                        const norm = (t: any) => String(t || '').replace(/\s+/g, ' ').trim().toLowerCase();
                        ...
                        for (const b of btns) {
                            ...
                            if (MARKERS.some(m => t === m || t.startsWith(m))) return true;
                        }
        ...
        for (let retry = 0; retry < 2 && !listOpened; retry++) {
            ...
            if (found) {
                ...
                try {
                    ...
                    await safeEval(frame, (node: any) => node.scrollIntoView({ block: 'center', inline: 'center', behavior: 'instant' }), handle, 5000);
                    const el = handle.asElement();
                    if (el) {
                        await el.click().catch(async () => { await safeEval(frame, (node: any) => node.click(), handle, 5000); });
        ...
        for (let retry = 0; retry < 2 && !netBankingVerified; retry++) {
            ...
            if (found) {
                const { handle, frame, page: fp } = found;
                try {
                    const info = await safeEval(frame, (node: any) => ({ tag: node.tagName.toLowerCase(), role: node.getAttribute('role'), selected: node.getAttribute('aria-selected') }), handle, 5000);
...
const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null) as any;
                                    ...
                                    if (url.includes('netbanking') || url.includes('banktransfer')) { verified = true; break; }
                                }
                            } catch (e) { }
                            if (!verified) await new Promise(r => setTimeout(r, 300));
                        }
        ...
        if (!netBankingVerified) {
            ...
            for (const { frame } of frames) {
                try {
                    const clicked = await safeEval(frame, () => {
                        const isVis = (el: any) => el.isConnected && el.getBoundingClientRect().width > 0 && el.getBoundingClientRect().height > 0;
                        const norm = (t: any) => String(t || '').replace(/\s+/g, ' ').trim().toLowerCase();
        ...
        while (Date.now() - startMonitorTime < 25000) {
            ...
            if (currentUrl.includes('getupgrade')) break;
