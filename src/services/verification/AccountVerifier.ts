            ...
            while (!phoneSuccess && attempts < maxAttempts) {
                ...
                try {
                    ...
                    for (const sel of codeSelectors) {
                        ...
                        if (smsCodeInput) { Logger.info(`âœ… Code input found: ${sel}`); break; }
                    }
                ...
                if (currentUrl2.includes('speedbump') || currentUrl2.includes('gaplustos')) {
                    ...
                    try {
                        ...
                        if (!tosClicked) {
                            ...
                            if (tosClicked) Logger.info(`âœ… TOS: Clicked via form submit`);
                        }
            ...
            if (isSuccess) {
                ...
                try {
                    ...
                    for (let wizardStep = 0; wizardStep < 5; wizardStep++) {
                        ...
                        if (!stepClicked) { Logger.info(`â„¹ï¸ No more wizard buttons at step ${wizardStep}`); break; }
                    } // END wizard for-loop
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        ...
                        if (txtRecord) { Logger.info(`ðŸ“ TXT Record (attempt ${attempt + 1}): ${txtRecord}`); break; }
...
private async waitForCheckoutFormToLoad(page: any, timeout = 35000): Promise<boolean> {
    ...
    async runSetup(email: string, password: string, headless: boolean = HEADLESS): Promise<{ success: boolean; error?: string }> {
        ...
        try {
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
                ...
                if (clicked) { Logger.info(`âœ… [runSetup] Clicked "Start a trial"`); await new Promise(r => setTimeout(r, 5000)); break; }
                await new Promise(r => setTimeout(r, 3000));
            }
                        ...
                        for (const btn of allBtns) {
                            ...
                            if (/^agree and continue$|^agree & continue$/i.test(txt)) {
                                ...
                                if (box && box.width > 0 && box.height > 0) {
                                    ...
                                    if (fb) { await page.mouse.click(fb.x+fb.width/2, fb.y+fb.height/2); Logger.info(`âœ… [runSetup] Clicked terms gate`); await new Promise(r => setTimeout(r, 4000)); break; }
                                }
                            }
                        }
                    } catch (e) { /* skip */ }
            ...
            for (let attempt = 1; attempt <= 5 && !addressSaved; attempt++) {
                        ...
                        for (const btn of btns) {
                            ...
                            if (/^(Save|Save address|Apply|OK|Done|Confirm|Continue|Next)$/i.test(txt)) {
                                ...
                                if (box && box.width > 0 && box.height > 0) {
                                    ...
                                    await new Promise(r => setTimeout(r, 200));
                                    const fb = await btn.boundingBox();
                                    if (fb) { await page.mouse.click(fb.x+fb.width/2, fb.y+fb.height/2); Logger.info(`ðŸ’¾ [runSetup] Save clicked: "${txt}"`); addressSaved = true; break; }
                                }
                            }
                        }
                    } catch(e){}
                ...
                if (!stillOpen) { Logger.info(`âœ… [runSetup] Address saved successfully`); break; }
                ...
                try {
                    ...
                    for (const btn of btns) {
                        ...
                        if (txt === 'add payment method') {
                            ...
                            addPaymentClicked = true; Logger.info(`âœ… [runSetup] Clicked "Add payment method"`); break;
                        }
                    }
                } catch(e){}
                    ...
                    try {
                        ...
                        for (const btn of btns) {
                            ...
                            if (txt === 'pay with netbanking') {
                                ...
                                await new Promise(r => setTimeout(r, 200));
                                await btn.click().catch(async () => { await btn.evaluate((el: any) => el.click()); });
                                netBankingClicked = true; Logger.info(`âœ… [runSetup] Clicked "Pay with NetBanking"`); break;
                            }
                        }
                    } catch(e){}
            ...
            for (const bank of banks) {
                for (const frame of page.frames()) {
                    ...
                    if (bankSelected) { Logger.info(`ðŸ¦ [runSetup] Bank selected: ${bank}`); break; }
                    try {
                    ...
                    for (const btn of btns) {
                        ...
                        if (/^(Save|Add|Done|Confirm|Save payment method)$/i.test(txt)) {
                            ...
                            if (box && box.width > 0 && box.height > 0) {
                                ...
                                await new Promise(r => setTimeout(r, 200));
                                await btn.click().catch(async () => { await btn.evaluate((el: any) => el.click()); });
                                Logger.info(`ðŸ’¾ [runSetup] Payment saved: "${txt}"`); break;
                            }
                        }
                    }
                } catch(e){}
                ...
                try {
                    ...
                    for (const btn of btns) {
                        ...
                        if (txt === 'checkout' || txt === 'agree and continue' || txt === 'agree & continue') {
                            ...
                            await new Promise(r => setTimeout(r, 200));
                            await btn.click().catch(async () => { await btn.evaluate((el: any) => el.click()); });
                            checkoutClicked = true; Logger.info(`âœ… [runSetup] Checkout clicked`); break;
                        }
                    }
                } catch(e){}
                ...
                for (const frame of page.frames()) {
                    try {
                        ...
                        for (const btn of btns) {
                            ...
                            if (txt === 'checkout' || txt === 'agree and continue' || txt === 'agree & continue') {
                                await btn.click().catch(async () => { await btn.evaluate((el: any) => el.click()); });
                                Logger.info(`âœ… [runSetup] Re-clicked checkout after popup`); break;
                            }
                        }
                    } catch(e){}
                }
            ...
            for (let i = 0; i < 25; i++) {
                if (page.url().includes('getupgrade')) { Logger.info(`âœ… [runSetup] Reached getupgrade page`); break; }
                await new Promise(r => setTimeout(r, 1000));
            }
    ...
    private async handleCheckout(page: any, email: string = '', password: string = ''): Promise<boolean> {
            ...
            for (const { frame } of frames) {
                try {
                    ...
                    if (Array.isArray(els) && els.length) {
                        for (const e of els) {
                            if (e && e.text) log(`ðŸ” ${why} â€” visible: text="${e.text}" role=${e.role} selected=${e.selected} expanded=${e.expanded}`);
                        }
                    }
                } catch (e) { }
            }
        ...
        try {
            ...
            if (bouncedAgain) {
                ...
                if (reauthOk) log(`âœ… Re-authenticated for checkout`);
                else warn(`âš ï¸ Re-auth failed, continuing anyway`);
                await humanDelay(2000, 3000);
            }
    ...
    private async enterIndianAddressInPopup(
        ...
        const formLoaded = await this.waitForCheckoutFormToLoad(page, 30000);
        if (formLoaded) log(`âœ… Payment page sections visible in frame(s)`);
        else warn(`âš ï¸ Payment page section wait timed out`);
            ...
            if (!stillHasInputs && anyFieldFilled) {
                ...
                for (const { frame } of trg) {
                    try {
                        ...
                        if (t2) { log(`âœ… Re-clicked "Add name and address" trigger (interactive): "${t2}"`); break; }
                    } catch (e) { }
                }
                await humanDelay(1500, 2500);
            } else {
    ...
    private async selectNetBankingAndCheckout(
        ...
        for (let retry = 0; retry < 2 && !netBankingVerified; retry++) {
            ...
            if (currentFrame && clickTargetHandle) {
                ...
                if (nbClicked) {
                    ...
                    if (netBankingVerified) log(`âœ… NetBanking selection verified`);
                }
            }
        ...
        if (popupPage) {
            ...
            const popupUrl = (() => { try { return (popupPage.url() || 'unknown').substring(0, 150); } catch (e) { return 'unknown'; } })();
        ...
        while (Date.now() - postPopupStart < 50000) {
            ...
            if (elapsed % 5000 < 1000) log(`🔗 Still waiting... ${Math.floor(elapsed/1000)}s elapsed — current: ${page.url().substring(0, 80)}`);
            await new Promise(r => setTimeout(r, 1000));
        }
