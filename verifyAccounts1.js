...
// CONFIGURE 2-STEP VERIFICATION SETTINGS
async function configure2StepVerification(page) {
    try {
        // Navigate to 2-Step Verification settings
        console.log(`🌐 Navigating to 2-Step Verification settings...`);
        await page.goto('https://admin.google.com/ac/security/2sv?hl=en', {
        ...
        await page.waitForFunction(() => {
            return document.body.innerText.includes('2-Step Verification') ||
                document.body.innerText.includes('Enforcement');
        }, { timeout: 15000 });

        // Check if "On" radio button exists
        console.log(`🔍 Looking for Enforcement radio buttons...`);
        const onRadioExists = await page.evaluate(() => {
            const labels = Array.from(document.querySelectorAll('label, span, div'));
            return labels.some(l => l.innerText.trim() === 'On');
        });

        if (!onRadioExists) {
            console.log(`⚠️  'On' radio button not found. Page might already be configured or layout changed.`);
            return;
        }

        // Click "On" radio button
        console.log(`🖱️  Clicking 'On' radio button...`);
        const clickedOn = await page.evaluate(() => {
            // Find the "On" label
            const labels = Array.from(document.querySelectorAll('label, span, div'));
            const onLabel = labels.find(l => l.innerText.trim() === 'On');

            if (onLabel) {
                // Find associated radio input
                let radio = onLabel.querySelector('input[type="radio"]');

                // If not inside, check parent or siblings
                if (!radio) {
                    radio = onLabel.parentElement?.querySelector('input[type="radio"]');
                }

                // If still not found, try finding by value or nearby
                if (!radio) {
                    const allRadios = Array.from(document.querySelectorAll('input[type="radio"]'));
                    // Usually there are 3 radios: Off, On, On from [date]
                    // "On" is typically the second one
                    radio = allRadios.find(r => {
                        const parent = r.parentElement;
                        return parent && parent.innerText.includes('On') && !parent.innerText.includes('from');
                    });
                }
        ...
        if (clickedOn) {
            ...
            console.log(`⚠️  Could not click 'On' radio button (might already be selected)`);
        }

        // Scroll down to see "New user enrollment period"
        console.log(`📜 Scrolling down to find enrollment period...`);
        await page.evaluate(() => {
            window.scrollBy(0, 400);
        });
        await new Promise(r => setTimeout(r, 1500));

        // Look for "New user enrollment period" dropdown
        console.log(`🔍 Looking for enrollment period dropdown...`);
        const dropdownFound = await page.evaluate(() => {
            const text = document.body.innerText;
            return text.includes('New user enrollment period') ||
                text.includes('enrollment period');
        });

        if (!dropdownFound) {
            console.log(`⚠️  Enrollment period section not found. Might not be visible yet.`);
            // Try scrolling more
            await page.evaluate(() => window.scrollBy(0, 300));
            await new Promise(r => setTimeout(r, 2000));
        }

        // Find and click the dropdown (usually a select or div with role="listbox")
        console.log(`🖱️  Opening enrollment period dropdown...`);
        const dropdownClicked = await page.evaluate(() => {
            // Look for select element first
            const selects = Array.from(document.querySelectorAll('select'));
            const enrollmentSelect = selects.find(s => {
                const parent = s.parentElement;
                return parent && parent.innerText.includes('enrollment period');
            });
            ...
            // If no select, look for Material Design dropdown
            const dropdowns = Array.from(document.querySelectorAll('[role="combobox"], [role="listbox"], div[class*="dropdown"]'));
            for (const dd of dropdowns) {
                const parent = dd.closest('div');
                if (parent && parent.innerText.includes('enrollment')) {
                    dd.click();
                    return true;
                }
            }
        ...
        if (dropdownClicked) {
            ...
            console.log(`🖱️  Selecting '6 months' option...`);
            const optionSelected = await page.evaluate(() => {
                // For select element
                const selects = Array.from(document.querySelectorAll('select'));
                for (const select of selects) {
                    const options = Array.from(select.options);
                    const sixMonthOption = options.find(o =>
                        o.text.includes('6') && o.text.toLowerCase().includes('month')
                    );
                    if (sixMonthOption) {
                        select.value = sixMonthOption.value;
                        select.dispatchEvent(new Event('change', { bubbles: true }));
                        return true;
                    }
                }

                // For Material Design dropdown options
                const options = Array.from(document.querySelectorAll('[role="option"], li, div[class*="option"]'));
                const sixMonthOption = options.find(o =>
                    o.innerText.includes('6') && o.innerText.toLowerCase().includes('month')
                );

                if (sixMonthOption && sixMonthOption.offsetParent !== null) {
                    sixMonthOption.click();
                    return true;
                }
            ...
            if (optionSelected) {
                ...
                console.log(`⚠️  Could not select '6 months' option`);
            }
        } else {
            console.log(`⚠️  Could not open enrollment period dropdown`);
        }
        ...
        if (saveClicked) {
            ...
            console.log(`⚠️  Save button not found or not clickable`);
        }

        console.log(`✅ 2-Step Verification configuration completed`);

    } catch (error) {
        console.error(`❌ Error in configure2StepVerification: ${error.message}`);
        throw error;
    }
}
    ...
    try {
        ...
        if (isTxtPage) {
            ...
            if (txtRecord) {
                ...
                // Extract subdomain from email: support@program57.lanovera.fun -> program57
                const fullDomain = email.split('@')[1]; // program57.lanovera.fun
                const parts = fullDomain.split('.');

                // Assuming format constraint: SUBDOMAIN.DOMAIN.TLD (e.g. program57.lanovera.fun)
                // We need to add record for 'program57', NOT '@' (which is lanovera.fun)
                // BUT Cloudflare API often takes the full name or relative.
                // If we get Zone ID for 'lanovera.fun', we should add record for 'program57'.

                // Let's deduce the zone and the record name
                // We'll search for the zone based on the last two parts (lanovera.fun)
                const rootDomain = parts.slice(-2).join('.'); // lanovera.fun
                const recordName = parts.slice(0, -2).join('.'); // program57 (or empty if root)

                const zoneId = await cloudflareService.getZoneId(rootDomain);

                if (zoneId) {
                    console.log(`🌍 Zone ID found for ${rootDomain}. Adding record for '${recordName}'...`);
        ...
        if (finalUrl.includes('admin.google.com')) {
            ...
            // Configure 2-Step Verification Settings
            try {
                console.log(`\n🔐 Configuring 2-Step Verification Settings...`);
                await configure2StepVerification(page);
                console.log(`✅ 2-Step Verification configured successfully!`);
            } catch (error) {
                console.log(`⚠️  2-Step Verification configuration failed: ${error.message}`);
