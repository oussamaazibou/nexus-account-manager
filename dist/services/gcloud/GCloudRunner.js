...
export class GCloudRunner {
    ...
    async runCommand(command, retryCount = 0) {
    ...
    async createProject(projectId, name = 'my first project', orgId) {
        try {
    ...
    async enableApis(projectId, apis = ['admin.googleapis.com', 'gmail.googleapis.com', 'siteverification.googleapis.com', 'cloudresourcemanager.googleapis.com', 'iam.googleapis.com', 'orgpolicy.googleapis.com']) {
        const apiList = apis.join(' ');
        await this.runCommand(`gcloud services enable ${apiList} --project ${projectId}`);
    }
    async verifyApis(projectId) {
        return await this.runCommand(`gcloud services list --enabled --project ${projectId} | grep -E "admin|gmail|siteverification"`);
    }
    async createServiceAccount(projectId, saName, displayName = 'Automation SA') {
        try {
    ...
    async getOrgId() {
        ...
        if (!output)
            throw new Error("No Organization ID found. Is the user part of an Org?");
    ...
    async getOrgIdFromProject(projectId) {
        const output = await this.runCommand(`gcloud projects get-ancestors ${projectId} --format="value(id)"`);
        if (!output)
            throw new Error(`No ancestors found for project ${projectId}`);
    ...
    async getUniqueId(saEmail, idFilename) {
    ...
    async createKey(projectId, saEmail, keyPath) {
