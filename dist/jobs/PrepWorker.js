...
export class PrepWorker {
    ...
    async processJob(data) {
        ...
        if (mode === 'phone-only') {
            ...
            if (!userPassword)
                throw new Error('Password required for phone-only mode');
        ...
        if (userPassword) {
            try {
                const { checkAndActivateCloudConsole } = await import('../../checkCloudConsole.cjs');
                await checkAndActivateCloudConsole(userEmail, userPassword, headless);
            }
            catch (consoleErr) {
        ...
        // key uploaded to S3 above is used on the next fetch instead of a stale
        // copy of the old key.
        try {
            const safeEmail = userEmail.replace('@', '_at_').replace(/\./g, '_');
            ...
            if (fs.existsSync(cachedKeyPath)) {
                ...
                Logger.info(`Replaced locally-cached SA key for ${userEmail} - new JSON will be used`);
            }
        }
        catch (cacheErr) {
            Logger.warn(`Could not clear cached SA key for ${userEmail}: ${cacheErr.message}`);
        }
        ...
        if (userPassword) {
            ...
            try {
                const { addDomainWideDelegation } = await import('../../addDomainWideDelegation.cjs');
                await addDomainWideDelegation(userEmail, userPassword, saEmail, browser, configDir);
                const { default: setupAuthenticator } = await import('../../setupAuthenticator.cjs');
                const secretKey = await setupAuthenticator(userEmail, userPassword, browser);
                if (secretKey) {
                    if (this.isSshConfigured) {
                        try {
                            await this.ssh.uploadSecretKey(userEmail, secretKey);
                        }
                        catch (sshErr) {
                ...
                const { default: configure2SVPolicy } = await import('../../configure2SVPolicy.cjs');
                await configure2SVPolicy(userEmail, userPassword, browser);
            }
            catch (browserError) {
    ...
    saveFailedAccount(email, password) {
        try {
