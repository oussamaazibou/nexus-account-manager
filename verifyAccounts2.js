...
const VERIFIED_FILE = 'result_accounts.txt';
...
async function verifyAccount(email, password) {
    ...
    try {
        ...
        if (finalUrl.includes('admin.google.com')) {
            ...
            // Remove from ACCOUNTS_FILE to prevent duplicates/confusion
            try {
                const currentAccounts = fs.readFileSync(ACCOUNTS_FILE, 'utf-8').split('\n');
                const cleanAccounts = currentAccounts.filter(line => !line.trim().startsWith(email));
                fs.writeFileSync(ACCOUNTS_FILE, cleanAccounts.join('\n'));
                console.log(`🗑️ Removed ${email} from ${ACCOUNTS_FILE}`);
            } catch (err) {
                console.error(`Error removing account from file: ${err.message}`);
            }
