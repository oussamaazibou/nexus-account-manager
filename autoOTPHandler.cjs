...
async function handleOTPIfRequested(page, email, timeout = 10000) {
    try {
        ...
        const otpInput = await page.waitForSelector(
            'input[name="totpPin"], input[type="tel"], input[placeholder*="code"], input[id*="otp"], input[id*="totp"], input[aria-label*="verification"]',
            { timeout: timeout }
        ).catch(() => null);
    ...
    await page.evaluateOnNewDocument((emailAddress) => {
        ...
        const observer = new MutationObserver((mutations) => {
            const otpInput = document.querySelector(
                'input[name="totpPin"], input[type="tel"], input[placeholder*="code"], input[id*="otp"]'
            );
